...
func _ready():
	...
	# Default item if none is assigned
	if item == null:
		item = "item 1" # Make sure this key exists in UPGRADES
	
	lblName.text = UpgradeDb.UPGRADES[item]["displayname"]
	lblDescription.text = UpgradeDb.UPGRADES[item]["type"]
	lblLevel.text = UpgradeDb.UPGRADES[item]["level"]
	if itemIcon is TextureRect:
